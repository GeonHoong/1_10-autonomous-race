...
**LiDAR 가변 ROI**
초기 테스트에서는 속도가 빨라질수록 벽이나 주변 물체를 주행 경로상의 장애물로 오인식하는 문제가 있었습니다. 이를 해결하기 위해 가변 ROI 로직을 도입했고, TTC(Time-to-Collision) 기반으로 장애물과의 상대속도·충돌 예상 시간을 함께 계산해 오인식으로 인한 오차를 줄였습니다. 실제 주행에서도 의도한 대로 잘 작동했습니다.
